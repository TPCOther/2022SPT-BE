 ...
 * @LastEditTime: 2022-07-05 09:09:05
...
public class RoleServiceImpl implements RoleService{
    ...
    @Override
    public BaseExecution<String> getControllerMenuNameListById(Long staffId) throws BaseExecuteException {
        BaseExecution<String> baseExecution=new BaseExecution<>();
        Long roleId=staffMapper.findRoleIdByStaffId(staffId);
        List<Long> menus=roleMenuMapper.findControllerMenuIdListByRoleId(roleId);
        List<String> menuNames=new ArrayList<>();
        for(Long controllerMenuId:menus)
        {
            menuNames.add(controllerMenuMapper.findControllerMenuName(controllerMenuId));
        }
        baseExecution.setEum(ExecuteStateEum.SUCCESS);
        baseExecution.setTList(menuNames);
        return baseExecution;
    }
